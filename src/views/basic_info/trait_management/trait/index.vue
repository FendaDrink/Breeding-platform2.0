  ...
  <div class="app-container" style="width: 100%;min-height: calc(100vh - 84px);background-color: #eeeeee;">
    <el-card>
      <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="98px">
        <el-form-item :label="$t('basic.label.traitName')" prop="traitName">
          <el-input v-model="queryParams.traitName" :placeholder="$t('basic.placeholder.traitName')" clearable @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item :label="$t('basic.label.fullName')" prop="fullName">
          <el-input v-model="queryParams.fullName" :placeholder="$t('basic.placeholder.fullName')" clearable @keyup.enter.native="handleQuery" />
        </el-form-item>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          ...
          <el-button type="primary" plain icon="Plus" @click="handleAdd" 
            v-hasPermi="['system:trait:add']">{{ $t('basic.button.add') }}</el-button>
        </el-col>
        <el-col :span="1.5">
        ...
        <el-col :span="1.5">
          <el-button type="warning" plain icon="Download" @click="handleExport" 
            v-hasPermi="['system:trait:export']">{{ $t('basic.button.export') }}</el-button>
        </el-col>
      ...
      <el-table :data="traitList" @selection-change="handleSelectionChange">
        ...
        <el-table-column :label="$t('basic.table.operate')" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            ...
            <el-tooltip :content="$t('basic.table.delete')" placement="top">
    ...
    <el-dialog :title="title" v-model="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="108px">
        <el-form-item :label="$t('basic.table.traitName')" prop="traitName">
          <el-input v-model="form.traitName" :placeholder="$t('basic.placeholder.traitName')" />
        </el-form-item>
        <el-form-item :label="$t('basic.table.fullName')" prop="fullName">
          <el-input v-model="form.fullName" :placeholder="$t('basic.placeholder.fullName')" />
        </el-form-item>
        <el-form-item :label="$t('basic.table.abbreviation')" prop="abbreviationName">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        traitName: [
          { required: true, message: (localStorage.getItem('lang') === 'zh-CN' || !localStorage.getItem('lang'))  ? '性状类型名称不能为空' : 'Trait type cannot be empty', trigger: "blur" }
        ],
        fullName: [
          { required: true, message: (localStorage.getItem('lang') === 'zh-CN' || !localStorage.getItem('lang'))  ? '全称不能为空' : 'Full name cannot be empty', trigger: "blur" }
        ],
        abbreviationName: [
          { required: true, message: (localStorage.getItem('lang') === 'zh-CN' || !localStorage.getItem('lang'))  ? '缩写不能为空' : 'Abbreviation cannot be empty', trigger: "blur" }
        ],
        createBy: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm((localStorage.getItem('lang') === 'zh-CN' || !localStorage.getItem('lang'))  ? '是否确认删除名称为'+" '"+traitNames+"' "+'的性状?' : 'Are you sure you want to delete the item named'+" '"+traitNames+"' "+"?").then(function () {        return delTrait(traitIds);
      }).then(() => {
        this.getList();
